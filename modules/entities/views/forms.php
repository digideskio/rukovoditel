...
<ol id="forms_tabs_ol" class="sortable_tabs sortable">
  ...
  <div class="cfg_form_tab">
      ...
      <div class="cfg_form_tab_heading">
        <table width="100%">
          <tr>
            <td><b><?php echo $tabs['name'] ?></b></td>
            <td align="right"><?php echo  button_icon_edit(url_for('entities/forms_tabs_form','id=' . $tabs['id']. '&entities_id=' . $_GET['entities_id'])) . ' ' . button_icon_delete(url_for('entities/forms_tabs_delete','id=' . $tabs['id'] . '&entities_id=' . $_GET['entities_id'])) ?></td>
          </tr>
        </table>
      </div>
      <?php endif ?>
...
';
$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type not in (" . fields_types::get_type_list_excluded_in_form(). ") and  f.entities_id='" . db_input($_GET['entities_id']) . "' and f.forms_tabs_id=t.id and f.forms_tabs_id='" . db_input($tabs['id']) . "' order by t.sort_order, t.name, f.sort_order, f.name");
