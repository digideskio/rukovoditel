...
<?php  
  $filters_query = db_query("select rf.*, f.name from app_reports_filters rf left join app_fields f on rf.fields_id=f.id where rf.reports_id='" . db_input($reports_info['id']) . "' order by rf.id");
...
?>
  <tr>
    ...
    <td><?php echo ($v['filters_condition']=='include' ? TEXT_CONDITION_INCLUDE : TEXT_CONDITION_EXCLUDE) ?></td>
    <td class="nowrap"><?php echo reports::render_filters_values($v['fields_id'],$v['filters_values']) ?></td>            
  </tr>
<?php endwhile?>  
</tbody>
</table>
