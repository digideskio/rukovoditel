...
{
  $itme_query = db_query("select * from app_entity_" . $current_entity_id . " e where e.id='" . db_input($current_item_id) . "' " . items::add_access_query_for_parent_entities($current_entity_id));
  
  if(!db_fetch_array($itme_query))
  {
    redirect_to('dashboard/page_not_found');
  }
...
//check path to entity  
  if($current_item_id==0)
  {
    ...
    {       
     redirect_to('items/info','path=' . $path_info['full_path'] . '/' . $current_entity_id);
    }
  }
}
else
